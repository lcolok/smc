...
function ProgressBar() {
	...
	this.render = function(opts) {
		...
		if (percent < 1) {
			...
			processBar =
				' ' + chalk`{${this.theme_color} ${cell}}` + chalk`{white ${empty}}`;
			status = 'Building';
			statusICON = '●';
			...
			function parseTime(d, u) {
				return u == 's' ? (d / 1000).toFixed(2) + 's' : d + 'ms';
			}
		...
		// 拼接最终文本
		var cmdText = chalk`{${this.theme_color} ${statusICON} ${
			this.description
		}}${processBar} ${status}(${(100 * percent).toFixed(0)}%) ${rate}`;
	...
	this.stepRender = function(consoleMode) {
		clearImmediate(__this.timer); //如果已经有一个正在播放的进度条,先把它清除掉
